...
const App = () => {
  ...
  const analysis = useMemo(() => {
    ...
    if (is3231) {
      // 3231 緯創：短線波段版買入標準
      if (totalBuyScore > 34) buySignal = { text: '強力買進 (Strong Buy)', color: 'text-emerald-400 font-bold' };
      else if (totalBuyScore > 26) buySignal = { text: '嘗試進場 (Try Buy)', color: 'text-cyan-400' };
      else if (totalBuyScore >= 20) buySignal = { text: '中性觀察', color: 'text-blue-400' };
      // < 20 分：觀望（預設值）
      
      // 霸王條款：只有在建議買入時（>34分）且斜率持續惡化時才需要「逆勢」警告
      // 如果斜率在改善（負值縮小），代表趨勢可能轉好，不顯示警告
      if (maSlope < 0 && !isSlopeImproving && totalBuyScore > 34) {
    ...
    if (is3231) {
      // 3231 緯創：短線波段版賣出標準
      if (totalSellScore > 35) sellSignal = { text: '清倉賣出 (Clear Out)', color: 'text-rose-500 font-bold' };
      else if (totalSellScore > 27) sellSignal = { text: '獲利調節 (Trim)', color: 'text-orange-400' };
      // <= 27 分：續抱（預設值）
    } else {
      // 6669：原版賣出標準
